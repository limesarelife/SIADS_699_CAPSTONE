# SIADS_699_CAPSTONE
 ...
 ![alt text](https://github.com/limesarelife/SIADS_699_CAPSTONE/blob/main/BANNER-TEST-AC.png)
    ...
    Next, we will be cloning an existing repository built with the Django Rest Framework. This repo contains an e-commerce application with functionalities that allow users to add items to a cart, get a list of all the items in the cart, and edit and delete items in the shopping cart.
    ...
    Application Name (select default)
    It appears that you are using Python (select Y)
    Default Region (select us-east-1)
    User Credentials (these are the credentials for your IAM user role credentials) and only will be asked for once.
    Platform and Platform Branch (select Python 3.8 running on 64bit Amazon Linux 2)
    CodeCommit (select No we are using GitHub 🙂)
    SSH (select Y)
    Key Pair (you will want to go with default and it will generate one for you but you can create a Key Pair once set up and use the named pair moving forward if you so choose, you can do this by going to the AWS Elastic Beanstalk console and in the EC2 instance and create an RSA key pair and note for use in this step.)
    ...
    Environment Name (select default, this refers to the name of your EB environment)
    DNS CNAME Prefix (select default for now, this prefix is the one to be used when setting up your domain name)
    Load balancer (select default which should be option 2 and type: Application)
    Spot Fleet Requests (select No)
    ...
    villagers_acnh.py: Combines all ACP Poll data per villager. pytrends_script.py: Utilizes each villager name and pulls historical number of searches on Google via PyTrends and related queries (searches) - DO NOT RUN unless you fetch cookies and place them in a gitignore and curl into the network.
    villagers_final_connect.py: Combines the resultant csv files from the villagers_acnh.py and pytrends_script.py
    villager_features.py: adds needed features for the villagers in order to create the user questionnaire for the information retrieval and similarity measure. For example, astrology signs and music genres are added in this script, resultant file is the villager_final.csv which is used in the Django application/project
    ...
    PyTrends to gather Google searches for ACNH villager popularity.
    Animal Crossing Player poll results to compare with PyTrends results
    ...
    Visual Package 1 - Stacked bar Charts Species & Personality compared to Species and Astrological Sign
    Visual Package 2 - Bubble/Scatter Chart Google vs ACNH Poll results compared by Species and Personality
    Visual Package 3 - Stacked Bar Chart Overall Popularity By Personality and Astrology
    ...
    User Information Retrieval System LSI vs Word Embeddings visual
